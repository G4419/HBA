...
void downsample_voxel(pcl::PointCloud<PointType>& pc_in,pcl::PointCloud<PointType>& pc_out, double voxel_size)
{
	if (voxel_size < 0.01)
		return;

	std::unordered_map<VOXEL_LOC, M_POINT> feature_map;
	size_t pt_size = pc_in.size();

	for (size_t i = 0; i < pt_size; i++)
	{
		PointType &pt_trans = pc_in[i];
		float loc_xyz[3];
		for (int j = 0; j < 3; j++)
		{
			loc_xyz[j] = pt_trans.data[j] / voxel_size;
	...
	pt_size = feature_map.size();
	// pc.clear();
	pc_out.resize(pt_size);

	size_t i = 0;
	for (auto iter = feature_map.begin(); iter != feature_map.end(); ++iter)
	{
		pc_out[i].x = iter->second.xyz[0] / iter->second.count;
		pc_out[i].y = iter->second.xyz[1] / iter->second.count;
		pc_out[i].z = iter->second.xyz[2] / iter->second.count;
		i++;
	}
}
